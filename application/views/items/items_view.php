<div class="col-sm-10" id="main">
	<?php
		...
		if (empty($items) ) {
			...
			echo '<a href='. base_url('items/new') .' class="btn btn-warning">Register Item Now.</a>';
			echo '</div>';
		}else {
			echo '<div id="content">';
			...
			$item_table = $this->table->set_heading('No.','Name','Quantity','Category','Description', 'Price','Status','Action');
			...
			foreach ($items as $item) {
				...
				$item_table = $this->table->add_row(
					$num, 
					$item->name, 
					$orderingLevel->getQuantity($item->id)->quantity,
					$item->category, 
					$item->description, 
